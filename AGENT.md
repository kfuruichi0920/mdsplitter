...
```
   {workDir}/
	 ...
	 |_ _out/                  // 出力フォルダ（カードファイル、トレース管理ファイル）
	 |_ _logs/                 // ログフォルダ（動作ログファイル）
	 |_ <inputfile>.txt/.md    // 入力ファイル
	 |_ <inputfile>_<label>.json  // カードファイル
	 |_ trace_<left>_<right>.json // トレース管理ファイル
```
...
パラメータファイル（`settings.json`）はアプリの挙動や出力先などを定義する。主な項目は以下の通り。
...
- 入力関連
  - `maxWarnSizeMB`: 警告を出すサイズ閾値（既定: 10）
  - `maxConfirmSizeMB`: 続行確認を行うサイズ閾値（既定: 50）
...
トレース管理ファイルは、2つのカードファイル間の関係（トレーサビリティ）を保持する。左右は単なる識別子で、上下関係を意味しない。
...
- **レイアウト**:
  - **ツールバー（2段構成）**:
      1.  **上段**: 
      以下を左から順に表示する。
        - テキスト(txt,md)開く ※アイコン表示する
        - 上書き保存 ※アイコン表示する
        - 名前を付けて保存 ※アイコン表示する
        - 現在開いているJSONファイル名 ※未保存の場合は、その状態が視覚的にわかるようにする。
      2.  **下段**: 
      以下を左から順に表示
        - 階層折畳 ※アイコン表示する
        - 階層展開 ※アイコン表示する
...
- **レイアウト**:
  - **ツールバー（2段構成）**:
      1.  **上段**: 
      以下を左から順に表示する。
        - アイコン（カードファイル(json)の開く、上書き保存、名前を付けて保存、トレース情報保存）。未保存の場合は、その状態が視覚的にわかるようにする。
        - 現在開いているJSONファイル名
      2.  **下段**: 
      以下を左から順に表示
        - 操作戻る
  ...
  - **カード表示エリア**:
    - 変換結果のカードリストパネル(### 6.4 カードリストパネル)を表示する。
    - カード表示エリアは水平・垂直方向へ任意に分割可能とする。分割した一つのエリアをパネルと称する。
    - カードリストパネルに、まだカードファイルの情報が読み込まれていない場合、ドラッグアンドドロップでカードファイルをオープンする。
    ...
    - 隣り合った左右のパネルの各カード間のトレーサビリティ情報を、視覚でわかるようにコネクタで接続した表示を行う。
      ...
      - トレーサビリティのコネクタ表示有無のトグルスイッチがONに設定されている場合に表示する。
