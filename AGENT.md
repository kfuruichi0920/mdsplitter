...
- **内部の情報管理方法**:
  - 各カードは階層関係(親子関係)、順序関係(兄弟関係における順序性)をリスト構造として管理する。
  - これらの情報は、ファイル読み込み時にメモリ読込する(操作性を確保するため)。
...
```
   {workDir}/
	 ...
	 |_ _input/                // 入力フォルダ
	 |	|_ _<inputfile>_yymmdd-hhmmss.txt/md    // 入力ファイルのコピー
	 |_ _out/                  // 出力フォルダ（カードファイル、トレーサビリティ情報ファイル）
	 |	|_ <inputfile>_<label>.json  // カードファイル
...
- 入力ファイル
  ...
  - 改行: `\n` / `\r\n` を許容し、内部では正規化する。
  - 入力サイズ: 10MB超は警告ダイアログを表示し、処理継続／キャンセルをユーザが選択可能。
  - 内容: 仕様／設計／設計経緯などのテキスト情報（画像は対象外）

- カードファイル
  - ファイル名: `<inputfile>_<label>.json`
  - 目的: 構造化されたカード情報を保持（JSON）
  - <label>は、ファイル保存時に指定。指定がなければ空欄とし`<inputfile>.json`となる。

- トレーサビリティ情報ファイル
  - ファイル名: `trace_<left>_<right>.json`
  - 目的: 2つのカードファイル間のトレーサビリティ情報を保持（JSON）
  - <left>と<right>はカードファイル名(拡張子なし)
...
- カードパネルはカード編集画面中のパネルに表示する。パネルの上部にはカードファイル名をタブ表示する。
...
- 詳細表示は以下を表示する
	...
	- 情報内容
	- 最終更新時刻
- コンパクト表示の場合はカード情報種別と、情報内容の１行目で表示可能な文が見える。（よってカードの高さは１行分）
...
- `header`
	...
	- `inputFilePath`: inputフォルダの入力ファイルの絶対パス
...
- `parent_id`:親カードID、なければ空欄
- `child_ids[]`:子カード(自分より1階層下)IDリスト、なければ空欄
	- `prev_id`:兄弟(同一階層)の自分より順番が前のカードID、なければ空欄
	- `next_id`:兄弟(同一階層)の自分より順番が次のカードID、なければ空欄
...
- `header`
	- `id`: 一意な識別子
	- `fileName`: ファイル名
	- `FilePath`: inputフォルダの入力ファイルの絶対パス
	- `updatedAt`（ISO 8601）
	- `memo`: 任意記述
- `body[]`: トレーサビリティ情報
