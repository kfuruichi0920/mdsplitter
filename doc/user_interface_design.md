...
カードパネルエリアは、水平方向、垂直方向に、スプリット操作履歴に基づく再帰的グリッド構造とする。(VSCodeのような分割の考え方)。分割されたグリッドを分割ノードを称する。初期状態では単一パネルで表示し、ユーザが水平/垂直分割を実行すると等価なカードビューを複製して別パネルへ配置する。分割されたパネルは閉じる操作で統合でき、分割履歴は undo/redo の対象とする。

> 実装補足: `src/renderer/store/panelLayoutStore.ts` が分割ノードの再帰構造とアクティブパネル状態を保持し、`src/renderer/components/SplitView.tsx` がノードツリーを再帰描画する。水平方向は左右分割、垂直方向は上下分割となり、パネル境界のドラッグによりサイズ比率を調整できる。
分割ノードは以下のエリアが表示される。
...
- 設定変更は即時にストアへ反映される設計とし、テーマ切替など即時効果のある項目は保存前でもプレビューできるようスイッチに `適用` ボタンを併設する。
