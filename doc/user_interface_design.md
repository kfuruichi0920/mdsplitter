...
**概要:**
- 2つのカードファイル間のトレース関係をマトリクス（行列）形式で表示・編集する専用ウィンドウ。
- メインウィンドウから独立して動作し、並行してトレース管理を行える。
- マトリクスウィンドウとメインウィンドウは IPC 通信でリアルタイムに同期。

**起動方法:**
- メインウィンドウで左右パネルにカードファイルを開いた状態で、ツールバーの「📊 マトリクス」ボタンをクリック。
- マトリクス起動ダイアログで左右ファイルを選択し、「開く」ボタンで新規ウィンドウを起動。
...
**ヘッダー:**
- 左ファイル名、右ファイル名を表示。
- 統計情報（総トレース数、未トレース左カード数、未トレース右カード数）を表示。
- 設定アイコン（⚙️）でマトリクス設定ダイアログを開く。

**ツールバー:**
- **エクスポート**: CSV/Excel 形式でマトリクスをエクスポート。メモ含有オプション付き。
- **フィルタリセット**: 全フィルタ条件をクリア。
- **デフォルト設定**: デフォルトの relation 種別と方向性を設定。

**フィルタパネル:**
- **カードID検索**: テキスト入力で左右カードを部分一致フィルタ。
- **タイトル検索**: テキスト入力で左右カードタイトルを部分一致フィルタ。
- **ステータスフィルタ**: draft/review/approved/deprecated のチェックボックスで絞り込み。
- **トレースフォーカス**: 特定カードとのトレース有無で行列をフォーカス表示。

**マトリクス本体:**
- **行**: 左カードファイルのカード。カードID、タイトル、ステータスを表示。
- **列**: 右カードファイルのカード。カードID、タイトルを表示。
- **セル**: トレース関係の有無、種別、方向性を視覚的に表示。
  - トレースあり: 種別に応じた色とアイコンで表示（例: trace=青、refines=緑、tests=オレンジ）。
  - トレースなし: 空セル（クリックで追加可能）。
  - 方向性: 矢印で表示（→ / ← / ↔）。

**セル操作:**
- **クリック**: トレース関係の追加/削除をトグル。
- **右クリック**: コンテキストメニューで種別・方向性・メモを編集。
  - 種別変更: trace/refines/tests/duplicates/satisfy/relate/specialize から選択。
  - 方向性変更: left_to_right/right_to_left/bidirectional から選択。
  - メモ編集: テキスト入力ダイアログでメモを編集。
  - メモ削除: 確認ダイアログ（設定で無効化可能）を経てメモを削除。

**デザイン特徴:**
- テーブルヘッダーは固定表示（スクロール時も常に表示）。
- セルは 32px × 32px の正方形。
- ホバー時に行列全体をハイライト。
- 選択中セルは青枠で強調。
- ライト/ダークモード対応。
...
**概要:**
- カード編集履歴を時系列で表示し、任意のバージョンに復元できるダイアログ。
- カード右クリックメニューまたはショートカットから起動。
...
**ヘッダー:**
- カードID、タイトル、現在のステータスを表示。
- 現在のバージョン番号と総バージョン数を表示。

**バージョン一覧:**
- 最新バージョンから古い順に表示。
- 各バージョンに以下を表示:
  - 操作種別アイコン（作成=➕、更新=✏️、削除=🗑️、復元=↩️）。
  - タイムスタンプ（YYYY-MM-DD HH:MM:SS）。
  - 変更サマリ（タイトル変更、本文変更、ステータス変更など）。
  - 復元元バージョンへのリンク（復元操作の場合のみ）。
- バージョンをクリックすると差分表示エリアに詳細を表示。

**差分表示エリア:**
- 選択したバージョンと直前バージョンの差分を視覚的に表示。
- タイトル、本文、ステータス、種別の変更をハイライト。
- 追加=緑、削除=赤、変更=黄色で色分け。
- 本文の差分は行単位で表示。

**フッター:**
- **復元ボタン**: 選択したバージョンにカードを復元。確認ダイアログを表示。
- **閉じるボタン**: ダイアログを閉じる。

**デザイン特徴:**
- モーダルダイアログとして最大幅 800px、最大高さ 80vh。
- バージョン一覧はスクロール可能。
- ライト/ダークモード対応。
...
**概要:**
- 同一階層の連続カードを1つのカードに統合するダイアログ。
- 統合後のプロパティを編集し、元カード削除とトレース引き継ぎのオプションを提供。
...
**ヘッダー:**
- 統合対象カードの一覧を表示（カードID、タイトル）。
- 統合条件のメッセージを表示（同一階層、連続カード、子カードなし）。

**統合後プロパティ編集エリア:**
- **カードID**: 既定は最初のカードIDだが編集可能。
- **タイトル**: 既定は最初のカードタイトルだが編集可能。空の場合は統合ボタンを無効化。
- **種別**: ドロップダウンで選択（heading/paragraph/bullet/figure/table/test/qa/other）。
- **ステータス**: ドロップダウンで選択（draft/review/approved/deprecated）。
- **本文**: テキストエリアで編集。既定は全カードの本文を改行区切りで結合。

**オプション設定エリア:**
- **元のカードを削除**: チェックすると統合後に元カードを削除。既定はチェック済み。
- **トレース情報を引き継ぐ**: チェックすると元カードのトレース関係を統合後カードに引き継ぐ。既定はチェック済み。

**フッター:**
- **統合ボタン**: 統合を実行。タイトルが空の場合は無効化。実行中は「統合中…」と表示。
- **キャンセルボタン**: ダイアログを閉じる。

**デザイン特徴:**
- モーダルダイアログとして最大幅 600px、最大高さ 70vh。
- 入力フィールドはライト/ダークモードで適切なコントラストを確保。
- 統合条件を満たさない場合はダイアログ内にエラーメッセージを表示。
