...
- トレースマトリクス機能: カードファイル間のトレース関係をマトリクス形式で表示・編集
- カード履歴管理機能: カード編集履歴の記録・表示・復元
...
- **プロジェクト管理（.msp）**:
  - プロジェクトファイル `.msp`（JSON）でカードファイル/トレースファイル/テーマ設定を束ねる。構造は `version`, `metadata`, `files.{cardFiles[],traceFiles[]}`, `settings.theme`。
  - プロジェクト保存: 現在のワークスペースからカード・トレースの選択状態を .msp に出力。保存先はファイルダイアログ（mspフィルタ）で指定。
  - プロジェクト読込: ダイアログから .msp を選択して読み込み、含まれるカードファイルをパネルに自動配置（複数パネルがある場合はラウンドロビンで割付）。トレースファイル一覧も読み込む。
  - 整合性チェック: .msp 内のトレースについて、left/right 参照カードファイルの存在とカードID単位の存在を検証し、エラー/警告を一覧表示。
  - 不整合なトレースファイルはダイアログ上で「除外」操作によりプロジェクトから一時的に外すことができる（再検証でエラー行も消去）。
  - UIはコンパクトレイアウト（小さいフォント/余白・ボタン）でプロジェクト名・説明・カードファイル選択・トレース一覧・結果表示を1画面で操作可能。
...
- **変換仕様**:
  ...
  - 変換方式 
    ...
    - LLMによる変換: LLM（OpenAI/Gemini/Ollama等のAPI設定を可能とする）を使用しシステムプロンプトに従いカード分割する。`temperature=0` を既定（冪等性確保）。
...
- **動作ログ仕様**:
  - 変換処理開始/終了/警告/エラーを記録。LLM使用時はプロバイダとトークン数を記録。

### 3.2. 非機能要件
 - 変換処理中も他タブ/カード編集操作を可能にする（非同期処理）。
 - 変換状況は、ステータスバーに進捗状況を表示する。
 - 変換処理はキャンセル可能。
...
- **検索/置換仕様**:
  - カードに対する検索/置換の範囲は、開いている全カードファイル、_input, _output フォルダとサブフォルダのカードファイルを選択可能。
  - カードに対する検索は文字列検索と、正規表現を選択可能とする。
  - 検索結果は、サイドバーの一覧表示する。一覧表示の１行を選択することで、該当のカードを表示する。もしカードファイルが開いていない場合は、アクティブパネルに開いて該当のカードを表示する。
  - 検索結果のサイドバーの一覧表示は、カーソルで上下でき、次々と該当カードの表示を切り替えることができる。キー操作の場合はCtrl+Enterで該当カードにジャンプできる。
...
- **マトリクス表示仕様**:
  - 2つのカードファイル間のトレーサビリティ関係をマトリクス（行列）形式で表示。
  - 左カードファイルを行、右カードファイルを列として配置。
  - セルにトレース関係の有無、種別、方向性を視覚的に表示。
  - 独立したウィンドウで動作し、メインウィンドウと並行して操作可能。

- **マトリクス編集仕様**:
  - セルクリックでトレース関係の追加/削除。
  - セル右クリックメニューで種別・方向性・メモの編集。
  - 複数セルの一括編集（予定）。
  - 編集内容はリアルタイムにトレースファイル（`trace_*.json`）へ保存。
  - メインウィンドウのコネクタ表示に即座に反映。

- **フィルタリング仕様**:
  - カードID、タイトル、ステータスによる行列のフィルタリング。
  - 特定カードとのトレース有無でフォーカス表示。
  - フィルタ条件の保存と復元（予定）。

- **統計表示仕様**:
  - 総トレース数、未トレースカード数を左右別に表示。
  - フィルタ適用後の統計も同時表示。

- **エクスポート仕様**:
  - CSV 形式でマトリクスをエクスポート。
  - Excel 形式（.xlsx）でマトリクスをエクスポート。
  - メモ含有オプション付き。
  - エクスポート範囲の選択（全体/フィルタ結果のみ）。

- **リアルタイム同期仕様**:
  - マトリクスウィンドウとメインウィンドウ間で IPC 通信。
  - マトリクスでの編集をメインウィンドウへブロードキャスト。
  - メインウィンドウからの変更もマトリクスへ反映（予定）。

### 5.2 非機能要件
- 性能要件:
  - マトリクス初期表示: 1,000×1,000セルで < 3s を目安。
  - セル編集応答: < 100ms。
  - エクスポート処理: 10,000セルで < 5s を目安。
- 信頼性要件:
  - マトリクスウィンドウクラッシュ時もメインウィンドウは継続動作。
  - 編集中の異常終了時はトレースファイルのバックアップから復元。
- 操作性要件:
  - ショートカットキーでのセル移動と編集を可能とする。
  - フィルタ結果の視覚的フィードバックを提供。

## 6. カード履歴管理機能
### 6.1 機能要件

- **履歴記録仕様**:
  - カード作成・更新・削除・復元の操作を自動的に履歴として記録。
  - 履歴は `_out/.history/{fileName}/{cardId}.json` として保存。
  - 各バージョンは `operation`（create/update/delete/restore）、`timestamp`、カードスナップショット、差分情報（before/after）を含む。
  - 復元操作自体も履歴に記録し、復元元バージョンを追跡可能。

- **履歴表示仕様**:
  - カード右クリックメニューまたはショートカットから履歴ダイアログを開く。
  - バージョン一覧を時系列で表示（新しい順）。
  - 各バージョンの操作種別、タイムスタンプ、変更サマリを表示。
  - 復元元バージョンの参照リンクを表示。

- **差分表示仕様**:
  - 任意の2バージョン間の差分を視覚的に表示。
...
- **復元仕様**:
  - 選択したバージョンにカードを復元。
  - 復元確認ダイアログを表示。
  - 復元後、現在の編集内容は新しい履歴バージョンとして記録。
  - 復元操作は Undo/Redo の対象外（履歴システムで管理）。

- **履歴管理仕様**:
  - 履歴ファイルのサイズ制限（予定）。
  - 古い履歴の自動圧縮またはアーカイブ（予定）。
  - 履歴の検索とフィルタリング（予定）。

### 6.2 非機能要件
- 性能要件:
  - 履歴記録: < 50ms（非同期処理）。
  - 履歴読み込み: 100バージョンで < 500ms を目安。
  - 差分計算: < 100ms。
- 信頼性要件:
  - 履歴ファイル破損時はユーザー通知とログ記録。
  - 履歴記録失敗時もカード編集操作は継続。
- 操作性要件:
  - 履歴ダイアログの応答性を確保。
  - 差分表示の視認性を重視。
...
### 6.1 全体レイアウト
- **メニューバー**:
1.  ファイル(F)(ファイルを開く、上書き保存、別名保存、アプリケーションの終了)
2.  編集(E)(Undo、Redo、設定)
...
- トレース種別表示フィルタ（ドロップダウン表示し、トレースタイプ毎にチェックボックスON/OFF）
- パネル分割（左右分割、上下分割）
...
- コネクタ(トレーサビリティ情報)をUI上から操作（作成、変更、削除）可能とする。
	- 作成: 左右の異なるカードパネル間で、カードを相手側カード上にドラッグアンドドロップすることでコネクタを生成可能とする。
	- 作成: カードパネルのカードを選択した状態で、相手側カード上のカードも同様に選択し、「トレース👉作成」「トレース👈作成」「トレース⇔作成」ボタン押下によりコネクタ接続する。これは複数同時に作成可能とする。
	- 削除: コネクタを選択して削除する。Ctrlで複数同時選択可能とする。
- カード移動操作が視覚的にわかるような視覚効果を具備する。
- 選択中のカードに対して、別カードパネルのトレーサビリティ関係にあるカードを、視覚的に識別可能とする。
...
- カード移動のドラッグ操作時は、兄弟位置へ挿入する場合に青い挿入ラインと「ここに挿入（前/後）」ラベルを表示し、子として挿入する場合は対象カード全体に半透明オーバーレイと「子として追加」バッジを表示して階層を明示する。

#### 6.4.3 保存操作
- `Ctrl+S` またはツールバーの「💾 保存」で編集中のカードファイルを上書きし、出力フォルダ（`_out/`）に同名JSONとして書き出す。保存対象が未変更の場合はガードメッセージを表示する。
- `Ctrl+Shift+S` やツールバーの「📝 別名保存」で任意のファイル名を指定して保存できる。保存後はタブタイトルとアクティブファイル名を新しいものへ更新し、以降の上書き保存対象も新しいファイルに切り替わる。
- ファイル名には `..`、スラッシュ、バックスラッシュなどのパストラバーサル文字を使用できない。拡張子が未指定の場合は `.json` を自動付与する。
...
## 99. 将来機能メモ（構想中の案であり仕様ではないため、読み飛ばすこと）
...
### カード編集機能
	大量のカードがあると重い。
	　→部分的に表示する。表示前後10カードを読み込む等。
	
	コンパクトモードのタイトルはIDとして使用する。コンパクトモードで表示するのはタイトル+本文(併せてXXX文字まで)
	選択カードのみを通常／コンパクトモード表示変更
	MarkdownのTable表記対応
	
	同一パネル内で、同一かつ連続選択中カードの統合機能
	最下位層 or "本文属性"(="タイトル"でない)のカード種別 の未トレースカード数の表示
	カード状態に"廃止"を追加。"廃止"は情報としては削除しないが、コネクタ接続不可。
	
	カードの右クリック（コンテクスト）の機能実装
		クリップボードにテキスト形式でコピー（複数同時選択可能）

	プロジェクトファイル機能
		以下を記録する
		・プロジェクトに属するカードファイル
		・プロジェクトに属するトレースファイル
		・前回の画面分割構成とオープンファイル情報
		カードを編集しトレースありのカードを削除 or 廃止した場合、警告する。
			→保存時にトレース情報を削除する。
		
	トレースのマトリクス編集機能
		別ダイアログでマトリクス編集。ダイアログオープン時に２ファイルを選択
		マトリクスのセル編集。
		ダイアログ閉じたら、コネクタ表示に反映。
		タイトルはホップアップで内容表示
	...
	検索は別ダイアログに見直し。
	検索は、API・MCP機能のクエリと共通化する。

### カード履歴管理機能（検討不十分、必要性含め再考）
	カード単位の変更履歴保存
		→変更履歴情報は別ファイルで管理したほうがよい。
		　カード表示したときに、履歴情報を表示したい場合は、ボタンを押してカード内に表示する仕組み。
