...
- [ ] P2-08 UI テスト用のサンプルカードファイルを作成の動作確認に利用できるよう整備する。選択中の分割パネルに読み込む機能を実装する。
- [ ] P2-09 各分割パネルが独立したカードビューを保持できるようにする。同じカードファイルが複数分割パネルで開かれている場合は、編集後に保存され場合は他分割パネルであってもカードファイルの内容が同期する仕組みを整備する。競合が発生する場合は、競合を破棄するか、上書き保存するか、別名で保存するかの処理とする。
- [ ] P2-10 左右分割したパネル間でトレーサビリティコネクタを描画する UI を実装し、仮データで接続表示を確認する。
- [ ] P2-11 コネクタ描画用のスタブデータとテストシナリオを用意し、レイアウトやインタラクションの実現可能性を検証する。
- [ ] P2-12 カード構造の詳細表示/コンパクト表示を切り替える UI を実装し、モード間での状態保持と描画性能を確認する。
- [ ] P2-13 カード階層表示と展開/折畳操作を実装し、階層状態がモード切替・再読込で保持されることを保証する。
- [ ] P2-14 カード選択操作（単一/複数選択）とキーボード操作の挙動を実装し、選択状態がストアと同期することを確認する。
- [ ] P2-15 選択したカードの移動操作（ドラッグ&ドロップ／ショートカット）を実装し、階層整合性とアンドゥ処理を整備する。
- [ ] P2-16 カード編集 UI（ダブルクリックによるインライン編集）を実装し、編集確定/キャンセル時のストア更新とバリデーションを検証する。
- [ ] P2-17 UI テスト用のサンプルカードファイルを作成し、表示モード・階層操作・編集操作の動作確認に利用できるよう整備する。
- [ ] P2-18 設定ビュー UI を設計し、テーマ/入出力/ログ/ワークスペース項目を編集できるモーダル（またはサイドパネル）を実装する。
- [ ] P2-19 設定変更を `settings.json` へ保存し再起動後も反映されることを統合テストで確認する。
