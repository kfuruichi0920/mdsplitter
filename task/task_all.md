...
## フェーズ3: カード変換機能（大枠→詳細）
- [x] P3-00 ツールバーの開くボタンよりtxt or md ファイルを読込み、変換方式を選択、その後変換する流れを作成する。
- [x] P3-01 固定ルール変換エンジンのプロトタイプを作成し、仕様書サンプルからカードの順序/階層が生成されることをスナップショットテストで確認する。
- [x] P3-02 LLM 変換用の API アダプタ抽象化を実装し、LLM 未接続時はスタブで安全に動作することを確認する。
- [x] P3-03 CP932 を含む文字コード自動判別と UTF-8 変換処理を実装し、ユニットテストでエッジケースを網羅する。
- [x] P3-04 10MB 超警告/200MB 超中断のハンドリングを実装し、モック入力でダイアログ挙動を確認する。
- [x] P3-05 変換キャンセルと進捗表示を実装し、UI 上でキャンセル復帰が正しく機能することを確認する。
...
- [x] P5-04 選択中カード(🔁アイコン)がONの場合、トレーサビリティ関係のあるカードの強調表示を実装する。複数パネル間でトレーサビリティ関係が数珠繋ぎとなる場合にも適用する。
- [x] P5-04b 選択中のカードパネルの選択中のカード表示と、非選択中のカードパネルの選択中のカード表示　は識別可能な表示に見直す。
- [x] P5-04c トレース関係種別の違いは、コネクタ色で識別可能とする。コネクタ色はカスタマイズ可能とする。コネクタの強調は線太で識別可能とする。
- [x] P5-05 トレース作成のトレース関係種別、トレース表示切り替えのトレース関係種別のボタンと機能を実装する。
- [x] P5-06 user_interface_design.mdの3.5.1 ツールバーのトレース表示切替を実装する。
- [x] P5-07 カードのトレース接合点の機能を実装する。
- [ ] P5-08 トレーサ操作ログと監査記録を実装し、操作ごとにログが残ることを確認する。

## フェーズ6: 検索・フィルタ・生産性機能
- [x] P6-01 全文/正規表現検索モジュールを実装し、範囲指定（全カードファイル/ディレクトリ）のテストを整備する。
- [x] P6-02 カード領域ツールバーの文字列フィルタ、およびカード種別情報フィルタ を実装する。フィルタ結果のカードは、階層補完表示を確認する。
- [x] P6-03 カード編集し未保存のカードは、視覚的に識別できるようにカード色を変更する。
- [x] P6-04 ログエリアのライブ更新とフィルタリング（レベル/キーワード）を実装し、性能要件を満たすことを確認する。
- [x] P6-05 ショートカットとコンテキストメニューの操作一覧をアプリ内ヘルプとして表示できるようにする。
